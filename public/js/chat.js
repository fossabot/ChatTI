...
var connect = function() {
    ...
    socket.onmessage = function(e) {
            ...
            switch(data.info) {
                case 'rejected':
                    ...
                    if(data.reason == 'length') {
                        message = 'Seu nome de usuário deve ter no mínimo 3 caracteres e no máximo 16 caracteres';
                    }

                    if(data.reason == 'format') {
                        message = 'Seu nome de usuário deve conter apenas caracteres alfanuméricos (números, letras e sublinhados)';
                    }
                    ...
                    if(data.reason == 'banned') {
                        message = 'Você foi banido do servidor por ' + data.time / 60 / 1000 + ' minutos. Você precisa esperar até que seja banido para poder se conectar novamente.';
                    }
                ...
                case 'success':
                    ...
                    updateBar('mdi-content-send', 'Digite sua mensagem aqui', false);
                    connected = true;
                ...
                case 'update':
                    showChat('info', null, data.user.oldun + ' mudou seu nome para ' + data.user.un);
                    clients[data.user.id] = data.user;
                    break;

                case 'connection':
                    var userip = data.user.ip ? ' [' + data.user.ip + ']' : '';
                    showChat('info', null, data.user.un + userip + ' está conectando ao servidor');
                ...
                case 'disconnection':
                    ...
                    if(data.user.un != null) {
                        showChat('info', null, data.user.un + userip + ' está desconectado do servidor');
                    }
                ...
                case 'spam':
                    showChat('global', null, 'Você precisa esperar 1 segundo entre as mensagens. Continuar enviando spam aos servidores pode fazer com que você seja banido. Aviso ' + data.warn + ' de 5');
                    break;
        ...
        if(data.type == 'global' || data.type == 'pm' || data.type == 'mention') {
            if(!focus) {
                unread++;
                document.title = '(' + unread + ') CHAT - DWeb';
    ...
    if(value.length > 0) {
        if(username === undefined) {
            ...
            switch(command[0].toLowerCase()) {
                ...
                default:
                    showChat('light', 'Error', 'Comando desconhecido, use /help para obter uma lista dos comandos disponíveis');
                    break;
            }
        } else {
...
function desktopNotif(message) {
    ...
    var notification = new Notification('Você recebeu uma nova mensagem', {

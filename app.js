...
chat.on('connection', function(conn) {
    log('socket', chalk.underline(conn.id) + ': conectado (' + conn.headers['x-forwarded-for'] + ')');
    rateLimit[conn.id] = 1;
    ...
    conn.on('data', function(message) {
        ...
        if(rateLimit[conn.id] < 1 && JSON.parse(message).type != 'delete' && JSON.parse(message).type != 'typing' && JSON.parse(message).type != 'ping') {
            ...
            if(clients[conn.id].warn < 6) {
                ...
                utils.sendToAll(clients, {type:'ban', extra:clients[conn.id].un, message:'Servidor banido ' + clients[conn.id].un + ' do servidor por 5 minutos para enviar spam aos servidores'});
    ...
    conn.on('close', function() {
        log('socket', chalk.underline(conn.id) + ': desconectado (' + clients[conn.id].ip + ')');
            ...
            if(user.role > 0) {
                if(data.type == 'global') {
                    ...
                    if(data.message != data.user) {
                        if(utils.checkUser(clients, data.message)) {
                            switch(data.type) {
                                case 'ban':
                                    ...
                                    if(!isNaN(time) && time > 0) {
                                        if(user.role > 1 && utils.getUserByName(clients, data.message).role == 0) {
                                            ...
                                            data.message = data.user + ' foi banido ' + data.message + ' do servidor para ' + time + ' minutos';
                                            return utils.sendToAll(clients, data);
                                        } else {
                                            data.message = 'Você não tem permissão para fazer isso';
                                            return utils.sendBack(clients, data, user);
                                ...
                                case 'kick':
                                    if(user.role > 1 && utils.getUserByName(clients, data.message).role == 0) {
                                        data.extra = data.message;
                                        data.message = data.user + ' kickado ' + data.message + ' do servidor';
                                    } else {
                                        data.message = 'Você não tem permissão para fazer isso';
                                        return utils.sendBack(clients, data, user);
                                    }
                        ...
                        data.message = 'Você não pode fazer isso consigo mesmo';
                        utils.sendBack(clients, data, user);
                    }
                }
            } else {
